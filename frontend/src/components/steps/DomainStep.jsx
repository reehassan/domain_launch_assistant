...
// DomainCard, not a flat bordered list.
//
// `changingDomain`: mirrors BrandStep's `changingBrand` — lets the
// founder swap their selected domain when they've navigated back to
// view this step. Only offered while project.status !== "READY": once
// the project's reached READY (DNS verified etc.), changing the domain
// out from under that would leave stale downstream state, so it's
// locked out past that point. Same frontend-only policy as BrandStep —
// DomainSelectView has no server-side guard against re-selection either.
...
export default function DomainStep({ project, onProjectUpdate }) {
  ...
  // Don't leave a stale "changing" UI open across a project switch or
  // once the selection actually changes under us.
  useEffect(() => {
    setChangingDomain(false);
  }, [project.id, project.selected_domain?.id]);
  ...
  const canChangeDomain = Boolean(project.selected_domain) && project.status !== "READY";
  const showSelectedCard = Boolean(project.selected_domain) && !changingDomain;
  const showFindDomainsButton = domains !== null && domains.length === 0;
  const showResultsGrid =
    domains && domains.length > 0 && (!project.selected_domain || changingDomain);
      ...
      {showResultsGrid && (
        <>
          {changingDomain && (
            <button
              type="button"
              onClick={() => setChangingDomain(false)}
